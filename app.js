...
const MANAGER_USERNAME     = '@FunPay_Safety'

// Замените на URL вашего бота-сервера: 'https://your-bot.railway.app'
...
function startDealPolling(id) {
  ...
  _pollInt = setInterval(async function() {
    ...
    if (local && local.step !== sv.step) {
      Object.assign(local, sv); saveState(); renderDeals()
      openDealDetail(id); showToast('📡 Статус обновлён!')
    } else if (!local) {
      state.deals.push(sv); saveState(); renderDeals()
    } else if (local) {
      var ch = ['status','step','hasPartner','buyerUsername','sellerUsername'].some(function(k){ return local[k] !== sv[k] })
      if (ch) { Object.assign(local, sv); saveState() }
    }
  }, 5000)
}
  ...
  document.querySelectorAll('.modal-overlay').forEach(el => {
    el.addEventListener('click', e => {
      if (e.target === el) {
        ...
        if (el.id === 'modalDealDetail') stopDealPolling()
      }
    })
  })
...
function renderDeals() {
  ...
  var _as=['pending','active','funded','escrow']
  if (dealsFilter==='active') deals = deals.filter(function(d){ return _as.indexOf(d.status)>=0 })
  else deals = deals.filter(function(d){ return _as.indexOf(d.status)<0 })
...
async function openDealDetail(id) {
  ...
  if (sv) {
    ...
    if (lc) Object.assign(lc, sv); else { state.deals.push(sv); }
    saveState()
  }
  ...
  var isBuyer   = d.dealType === 'buy'
  var uname     = (tg && tg.initDataUnsafe && tg.initDataUnsafe.user && tg.initDataUnsafe.user.username) || 'user'
  var stepIdx   = Math.min(d.step || 0, 4)
  var cancelled = d.status === 'cancelled' || d.status === 'disputed'
  var completed = d.status === 'completed' || stepIdx >= 4
  var comm   = (parseFloat(d.amount) * 0.01).toFixed(2)
  var total  = (parseFloat(d.amount) * 1.01).toFixed(2)
  var bal    = ((state.balances[d.currency] || 0)).toFixed(2)
  var created = new Date(d.createdAt).toLocaleString('ru-RU')

  // Progress bar
  var progDots = ''
  STEPS.forEach(function(s,i) {
    var done = i <= stepIdx && !cancelled
    var curr = i === stepIdx && !cancelled
    progDots += '<div class="progress-dot ' + (done?'done':'') + ' ' + (curr?'current':'') + '"></div>'
    if (i < STEPS.length-1) progDots += '<div class="progress-line ' + (i<stepIdx&&!cancelled?'done':'') + '"></div>'
  })
  var progLabels = ''
  STEPS.forEach(function(s,i) {
    progLabels += '<span class="progress-label ' + (i<=stepIdx&&!cancelled?'done':'') + '">' + s + '</span>'
  })
  var prog = '<div class="deal-progress" style="margin:0">' + progDots + '</div><div class="progress-labels" style="margin-top:4px">' + progLabels + '</div>'

  // Participants
  var myBadge   = '<span class="link-blue">@' + uname + ' <span class="you-badge">Вы</span></span>'
  var noneTag   = '<span class="text-dim">Покупатель не присоединился</span>'
  var buyerDisp  = isBuyer  ? myBadge : (d.buyerUsername  ? '<span class="link-blue">@'+d.buyerUsername+'</span>'  : noneTag)
  var sellerDisp = !isBuyer ? myBadge : (d.sellerUsername ? '<span class="link-blue">@'+d.sellerUsername+'</span>' : '<span class="text-dim">Продавец не задан</span>')

  // Status text
  var statusDisp = STATUS_LABEL[d.status] || 'Активна'
  if (!cancelled && !completed && stepIdx===3) statusDisp = 'У менеджера'
  if (!cancelled && stepIdx===2) statusDisp = 'Оплачена'

  var secTitle = 'СЕЙЧАС'
  var secCls   = 'dd-now-box'
  var nowText  = ''
  var actions  = ''

  if (cancelled) {
    secCls  = 'dd-now-box dd-now-cancelled'
    nowText = d.status==='disputed'
      ? '⚠️ Открыт спор. Свяжитесь с менеджером <a href="https://t.me/FunPay_Safety" class="link-blue">@FunPay_Safety</a> для урегулирования.'
      : '❌ Сделка отменена.'
  } else if (completed) {
    secTitle = 'ГОТОВО'
    secCls   = 'dd-now-box dd-now-done'
    ...
    if (stepIdx===0) {
      ...
      actions = '<button class="btn-primary btn-block" onclick="shareDeal(''+d.id+'')">◎ Поделиться сделкой</button><button class="btn-secondary btn-block" onclick="cancelDeal(''+d.id+'')">Отменить сделку</button>'
    } else if (stepIdx===1) {
      nowText = 'Покупатель в сделке. Ждём, когда он оплатит. После оплаты вы передадите подарок менеджеру <a href="https://t.me/FunPay_Safety" class="link-blue">@FunPay_Safety</a>.'
      actions = '<button class="btn-primary btn-block" onclick="shareDeal(''+d.id+'')">◎ Поделиться сделкой</button><button class="btn-secondary btn-block" onclick="cancelDeal(''+d.id+'')">Отменить сделку</button>'
    } else if (stepIdx===2) {
      secTitle = 'ВАШ ШАГ'
      nowText  = 'Передайте подарок ТОЛЬКО менеджеру <a href="https://t.me/FunPay_Safety" class="link-blue">@FunPay_Safety</a> — никогда напрямую покупателю. После передачи нажмите «Передал менеджеру».'
      actions  = '<button class="btn-primary btn-block" onclick="showTransferConfirm(''+d.id+'')">Передал менеджеру</button><a href="https://t.me/FunPay_Safety" class="btn-secondary btn-block" target="_blank" style="text-align:center;text-decoration:none;display:block;box-sizing:border-box">💬 Открыть чат с менеджером</a>'
    } else if (stepIdx===3) {
      nowText = 'Подарок у менеджера <a href="https://t.me/FunPay_Safety" class="link-blue">@FunPay_Safety</a>. Ждём, когда покупатель проверит и подтвердит получение — после этого деньги придут к вам.'
    }
  } else {
    // BUYER
    if (stepIdx<=1) {
      secTitle = stepIdx===0 ? 'СЕЙЧАС' : 'ВАШ ШАГ'
      var hasBal = (state.balances[d.currency]||0) >= parseFloat(total)
      if (stepIdx===0) {
        nowText = 'Ожидаем продавца. Сделка готовится.'
      } else {
        nowText = 'Оплатите сделку, чтобы продолжить.'
        actions = hasBal
          ? '<button class="btn-primary btn-block" onclick="buyerPayDeal(''+d.id+'')">💳 Оплатить с баланса ('+bal+' '+d.currency+')</button>'
          : '<div class="dd-low-balance">⚠️ Недостаточно средств. Пополните баланс.</div><button class="btn-secondary btn-block" onclick="closeModal('modalDealDetail');switchTab('profile')">Пополнить баланс</button>'
      }
    } else if (stepIdx===2) {
      nowText = 'Оплата принята! Продавец передаёт товар менеджеру <a href="https://t.me/FunPay_Safety" class="link-blue">@FunPay_Safety</a>. Ожидайте.'
    } else if (stepIdx===3) {
      secTitle = 'ВАШ ШАГ'
      nowText  = 'Свяжитесь с менеджером <a href="https://t.me/FunPay_Safety" class="link-blue">@FunPay_Safety</a> и проверьте товар. Если всё хорошо — нажмите «Подтвердить получение товара».'
      actions  = '<button class="btn-primary btn-block" onclick="showReceiptConfirm(''+d.id+'')">Подтвердить получение</button>'
    }
  }

  var payCard = ''
  if (isBuyer && stepIdx===1) {
    payCard = '<div class="dd-card dd-payment-card"><div class="dd-pay-row"><span>К оплате</span><span>'+d.amount+' '+d.currency+'</span></div><div class="dd-pay-row"><span>Комиссия 1%</span><span>'+comm+' '+d.currency+'</span></div><div class="dd-pay-row dd-pay-total"><span>Итого</span><span>'+total+' '+d.currency+'</span></div></div>'
  }

  var successCard = ''
  if (completed) {
    successCard = '<div class="dd-success-card"><svg width="48" height="48" viewBox="0 0 48 48" fill="none" style="display:block;margin:0 auto 8px"><circle cx="24" cy="24" r="24" fill="#22c55e22"/><path d="M14 24l8 8 12-16" stroke="#22c55e" stroke-width="3" stroke-linecap="round" stroke-linejoin="round"/></svg><div class="dd-success-text">Сделка успешно завершена</div></div>'
  }

  document.getElementById('dealDetailContent').innerHTML =
    '<div class="dd-header">' + svgIcon(d.currency,44) + '<div><div class="dd-amount">'+d.amount+' '+d.currency+'</div><div class="dd-desc">'+(d.description||(isBuyer?'Покупка':'Продажа'))+'</div></div></div>' +
    '<div class="dd-card"><div class="dd-card-title">СТАТУС СДЕЛКИ</div><div class="dd-step-title">Шаг '+(stepIdx+1)+' из 5 — '+STEPS[stepIdx]+'</div>' + prog + '<div class="dd-secure-tag">Безопасно через Telegram</div></div>' +
    '<div class="dd-card"><div class="dd-info-row"><span class="dd-label">Статус</span><span>'+statusDisp+'</span></div><div class="dd-info-row"><span class="dd-label">Продавец</span><span>'+sellerDisp+'</span></div><div class="dd-info-row"><span class="dd-label">Покупатель</span><span>'+buyerDisp+'</span></div><div class="dd-info-row dd-last-row"><span class="dd-label">Создана</span><span>'+created+'</span></div></div>' +
    '<div class="' + secCls + '"><div class="dd-now-label">' + secTitle + '</div><div class="dd-now-text">' + nowText + '</div></div>' +
...
async function buyerPayDeal(id) {
  var d = state.deals.find(function(x){return x.id===id}); if(!d) return
  ...
  apiCall('/api/deals/'+id,'PUT',{action:'buyer_paid',actorId:tg&&tg.initDataUnsafe&&tg.initDataUnsafe.user&&tg.initDataUnsafe.user.id})
...
async function doTransferConfirm() {
  ...
  var d = state.deals.find(function(x){return x.id===id}); if(!d) return
  d.step=3; d.status='escrow'; saveState()
  apiCall('/api/deals/'+id,'PUT',{action:'seller_transferred',actorId:tg&&tg.initDataUnsafe&&tg.initDataUnsafe.user&&tg.initDataUnsafe.user.id})
  setTimeout(function(){openDealDetail(id)},200)
...
async function doReceiptConfirm() {
  ...
  apiCall('/api/deals/'+id,'PUT',{action:'buyer_confirmed',actorId:tg&&tg.initDataUnsafe&&tg.initDataUnsafe.user&&tg.initDataUnsafe.user.id})
...
async function cancelDeal(id) {
  ...
  if (d.status==='funded'||d.status==='escrow') {
    ...
    apiCall('/api/deals/'+id,'PUT',{action:'dispute',actorId:tg&&tg.initDataUnsafe&&tg.initDataUnsafe.user&&tg.initDataUnsafe.user.id})
    stopDealPolling(); closeModal('modalDealDetail'); renderDeals()
    showToast('⚠️ Спор открыт — обратитесь к @FunPay_Safety'); return
  }
  ...
  apiCall('/api/deals/'+id,'PUT',{action:'cancel',actorId:tg&&tg.initDataUnsafe&&tg.initDataUnsafe.user&&tg.initDataUnsafe.user.id})
  stopDealPolling(); closeModal('modalDealDetail'); renderDeals()
...
async function acceptJoinDeal() {
  ...
  var serverDeal = await apiCall('/api/deals/' + info.id)
  var creator   = (serverDeal && serverDeal.creatorUsername) || info.creatorUsername || null
  var creatorId = (serverDeal && serverDeal.creatorId) || null

  var deal = {
    id: info.id, dealType: myRole,
    amount:   (serverDeal && serverDeal.amount)   || info.amount,
    currency: (serverDeal && serverDeal.currency) || info.currency,
    description: (serverDeal && serverDeal.description) || 'Присоединился через ссылку',
    status: 'active', step: Math.max((serverDeal && serverDeal.step)||0, 1),
    createdAt: (serverDeal && serverDeal.createdAt) || Date.now(),
    creatorId: (serverDeal && serverDeal.creatorId) || creatorId,
    creatorUsername: creator,
    buyerId:       myRole==='buy'  ? uid   : ((serverDeal && serverDeal.buyerId)  || creatorId),
    buyerUsername: myRole==='buy'  ? joiner : ((serverDeal && serverDeal.buyerUsername) || creator),
    sellerId:      myRole==='sell' ? uid   : ((serverDeal && serverDeal.sellerId) || creatorId),
    sellerUsername:myRole==='sell' ? joiner : ((serverDeal && serverDeal.sellerUsername) || creator),
    hasPartner: true
  }
  state.deals.push(deal); saveState(); renderDeals(); renderStats()
  apiCall('/api/deals/' + info.id + '/join', 'PUT', { joinerId:uid, joinerUsername:joiner, myRole:myRole })
  closeModal('modalJoinDeal'); switchTab('deals')
