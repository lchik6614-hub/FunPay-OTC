...
function renderProfile() {
  ...
  if (user) {
    ...
    nameEl.textContent = n + ' | 100+ reviews'
...
function shareDeal(id) {
  const d = state.deals.find(x=>x.id===id); if (!d) return
  const typeLabel = d.dealType==='buy'?'покупка':'продажа'
  const dealUrl = `https://t.me/FunpaySaf_Robot?start=DEAL-${d.id}`
...
function createDeal() {
  ...
  state.deals.push(deal); state.stats.active++
...
function renderWalletCurrencyTabs() {
  ...
  if (special === 'stars') {
    // STARS — пополнение через Telegram Stars
    walletCard.innerHTML = `
      ...
      <div class="wallet-special-info">
        ...
        <div class="wallet-special-text">
          ...
          <span>Пополнение производится через встроенную оплату Telegram Stars</span>
        </div>
      </div>
...
function openStarsPayment() {
  ...
  try {
    if (tg) {
      tg.openTelegramLink('https://t.me/FunpaySaf_Robot?start=buy_stars')
    }
  } catch(e) {}
