...
function openDealDetail(id) {
  ...
  const isBuyer   = d.dealType === 'buy'
  const uname     = tg?.initDataUnsafe?.user?.username || 'user'
  const stepIdx   = Math.min(d.step || 0, 4)
  const cancelled = d.status === 'cancelled'
  const completed = d.status === 'completed' || stepIdx >= 4
  const comm  = (parseFloat(d.amount) * 0.01).toFixed(2)
  const total = (parseFloat(d.amount) * 1.01).toFixed(2)
  ...
  STEPS.forEach((s, i) => {
    const done = i <= stepIdx && !cancelled
    const curr = i === stepIdx && !cancelled
    prog += `<div class="progress-dot ${done?'done':''} ${curr?'current':''}"></div>`
    if (i < STEPS.length - 1) prog += `<div class="progress-line ${i < stepIdx && !cancelled ? 'done' : ''}"></div>`
  })
  prog += `</div><div class="progress-labels" style="margin-top:4px">${STEPS.map((s,i) =>
    `<span class="progress-label ${i <= stepIdx && !cancelled ? 'done' : ''}">${s}</span>`).join('')}</div>`

  // Participants display
  const myBadge = `<span class="link-blue">@${uname} <span class="you-badge">Вы</span></span>`
  const noneTag = `<span class="text-dim">Не задан</span>`
  const buyerDisplay  = isBuyer  ? myBadge : (d.buyerUsername  ? `<span class="link-blue">@${d.buyerUsername}</span>`  : noneTag)
  const sellerDisplay = !isBuyer ? myBadge : (d.sellerUsername ? `<span class="link-blue">@${d.sellerUsername}</span>` : noneTag)

  // Role-specific actions and instruction
  let nowText = '', mainActions = ''

  if (cancelled) {
    nowText = '❌ Сделка была отменена.'
  } else if (completed) {
    nowText = '🎉 Сделка успешно завершена!'
  } else if (isBuyer) {
    if (stepIdx === 0 && !d.hasPartner) {
      nowText = 'Поделитесь ссылкой с продавцом. После того как он присоединится — нажмите «Продавец подтверждён».'
      mainActions = `
        <button class="btn-primary btn-block" onclick="shareDeal('${d.id}')">🔗 Поделиться сделкой</button>
        <button class="btn-success btn-block" onclick="buyerMarkSellerJoined('${d.id}')">✅ Продавец подтверждён</button>`
    } else if (stepIdx <= 1) {
      nowText = 'Продавец присоединился. Оплатите сделку с баланса, чтобы продолжить.'
      mainActions = `<button class="btn-primary btn-block" onclick="payFromBalance('${d.id}')">💳 Оплатить с баланса</button>`
    } else if (stepIdx === 2) {
      nowText = `Оплата принята! Продавец должен отправить подарок на @Funpay_Maneger (${d.amount} ${d.currency}). Ожидайте уведомления.`
    } else if (stepIdx === 3) {
      nowText = 'Подарок передан в эскроу. Убедитесь, что всё верно, и подтвердите получение.'
      mainActions = `<button class="btn-success btn-block" onclick="buyerConfirmReceipt('${d.id}')">✅ Подтвердить получение</button>`
    }
  } else {
    // Seller
    if (stepIdx <= 1) {
      nowText = 'Вы присоединились к сделке. Когда покупатель оплатит — нажмите кнопку ниже.'
      mainActions = `<button class="btn-success btn-block" onclick="sellerMarkPaid('${d.id}')">✅ Покупатель оплатил</button>`
    } else if (stepIdx === 2) {
      nowText = `Покупатель оплатил! Отправьте подарок на аккаунт @Funpay_Maneger ровно ${d.amount} ${d.currency}. После отправки нажмите кнопку.`
      mainActions = `<button class="btn-success btn-block" onclick="sellerConfirmEscrow('${d.id}')">✅ Подарок отправлен в эскроу</button>`
    } else if (stepIdx === 3) {
      nowText = 'Подарок в эскроу. Ожидаем подтверждения покупателя. После этого получите оплату.'
    }
  }

  const cancelBtn = (!cancelled && !completed)
  ...
  document.getElementById('dealDetailContent').innerHTML = `
    <div class="dd-header">
      ${svgIcon(d.currency, 44)}
      <div>
        <div class="dd-amount">${d.amount} ${d.currency}</div>
        <div class="dd-desc">${d.description || (isBuyer ? 'Покупка' : 'Продажа')}</div>
      </div>
    </div>

    <div class="dd-card">
      <div class="dd-card-title">СТАТУС СДЕЛКИ</div>
      <div class="dd-step-title">Шаг ${stepIdx + 1} из 5 — ${STEPS[stepIdx]}</div>
      ${prog}
      <div class="dd-secure-tag">🔒 Безопасно через Telegram</div>
    </div>

    <div class="dd-card">
      <div class="dd-info-row"><span class="dd-label">Статус</span><span>${STATUS_LABEL[d.status] || 'Активна'}</span></div>
      <div class="dd-info-row"><span class="dd-label">Тип</span><span>${isBuyer ? '🟢 Покупка' : '🔴 Продажа'}</span></div>
      <div class="dd-info-row"><span class="dd-label">Продавец</span><span>${sellerDisplay}</span></div>
      <div class="dd-info-row"><span class="dd-label">Покупатель</span><span>${buyerDisplay}</span></div>
      <div class="dd-info-row dd-last-row"><span class="dd-label">Создана</span><span>${created}</span></div>
    </div>

    ${!cancelled ? `
    <div class="dd-now-box">
      <div class="dd-now-label">СЕЙЧАС</div>
      <div class="dd-now-text">${nowText}</div>
    </div>
    <div class="dd-actions">
      ${mainActions}
      ${cancelBtn}
    </div>` : `
    <div class="dd-now-box dd-now-cancelled">
      <div class="dd-now-label">СТАТУС</div>
      <div class="dd-now-text">❌ Сделка отменена</div>
    </div>`}

    <div class="dd-card dd-payment-card">
      <div class="dd-pay-row"><span>К оплате</span><span>${d.amount} ${d.currency}</span></div>
      <div class="dd-pay-row"><span>Комиссия 1%</span><span>${comm} ${d.currency}</span></div>
      <div class="dd-pay-row dd-pay-total"><span>Всего</span><span>${total} ${d.currency}</span></div>
      <div class="dd-pay-row ${parseFloat(bal) < parseFloat(total) ? 'dd-pay-low' : ''}">
        <span>Ваш баланс</span><span>${bal} ${d.currency}</span>
      </div>
    </div>
  `
...
// ─── DEAL ACTIONS ─────────────────────────────────────────────────────────────
function buyerMarkSellerJoined(id) {
  const d = state.deals.find(x => x.id === id); if (!d) return
  d.hasPartner = true; d.step = 1; d.status = 'active'; state.stats.active++
  saveState(); renderDeals(); renderStats()
  setTimeout(() => openDealDetail(id), 100)
  showToast('✅ Продавец подтверждён!')
}

function buyerConfirmReceipt(id) {
  const d = state.deals.find(x => x.id === id); if (!d) return
...
function sellerMarkPaid(id) {
  const d = state.deals.find(x => x.id === id); if (!d) return
  if (d.status !== 'active') { d.status = 'active'; state.stats.active = (state.stats.active || 0) + 1 }
  d.step = 2; saveState()
  setTimeout(() => openDealDetail(id), 100)
  showToast('Отлично! Теперь отправьте подарок в эскроу')
}

function sellerConfirmEscrow(id) {
  const d = state.deals.find(x => x.id === id); if (!d) return
  d.step = 3; saveState()
  setTimeout(() => openDealDetail(id), 100)
  showToast('Подарок отправлен — ожидаем подтверждение покупателя')
}
...
function payFromBalance(id) {
  ...
  d.status='active'; d.step=2; state.stats.active++
...
function createDeal() {
  ...
  const _uname = tg?.initDataUnsafe?.user?.username || 'user'
  const deal = {
    ...
    buyerUsername:  newDeal.dealType==='buy'  ? _uname : null,
    sellerUsername: newDeal.dealType==='sell' ? _uname : null,
    hasPartner: false
  }
...
function acceptJoinDeal() {
  if (!_joinDealInfo) return
  const info = _joinDealInfo

  const existing = state.deals.find(x => x.id === info.id)
  if (existing) {
    closeModal('modalJoinDeal')
    switchTab('deals')
    setTimeout(() => openDealDetail(info.id), 200)
    showToast('Открываем сделку...')
    _joinDealInfo = null
    return
  }

  const joiner  = tg?.initDataUnsafe?.user?.username || 'user'
  const myRole  = info.dealType === 'sell' ? 'buy' : 'sell'
  const creator = info.creatorUsername || null
  ...
  const deal = {
    ...
    description:    'Присоединился через ссылку',
    status:         'pending',
    step:           0,
    createdAt:      new Date().toISOString(),
    buyerUsername:  myRole === 'buy'  ? joiner : creator,
    sellerUsername: myRole === 'sell' ? joiner : creator,
    hasPartner:     true
  }
  state.deals.push(deal)
  state.stats.total++
  ...
  closeModal('modalJoinDeal')
  switchTab('deals')
  setTimeout(() => openDealDetail(info.id), 300)
