...
// API_BASE: '' = relative (same server), null = disabled, 'http://...' = absolute
const API_BASE = (typeof window !== 'undefined' && 'FUNPAY_API_BASE' in window)
  ? window.FUNPAY_API_BASE
  : null;

async function apiCall(path, method, body) {
  if (API_BASE === null || API_BASE === undefined) return null; // сервер не настроен
  try {
    ...
    if (stepIdx <= 1) {
      if (stepIdx === 0) {
        ...
        actHtml  = hasBal
          ...
          + '<button class="btn-primary btn-block" onclick="closeModal(\'modalDealDetail\');switchTab(\'wallets\')">&#128179; Пополнить кошелёк</button>';
      }
...
async function buyerPayDeal(id) {
  ...
  if (curBal < total) { showToast('Недостаточно средств — переходим в кошелёк'); closeModal('modalDealDetail'); switchTab('wallets'); return; }
