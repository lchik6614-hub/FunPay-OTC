...
function shareDeal(id) {
  const d = state.deals.find(x=>x.id===id); if (!d) return
  const typeLabel = d.dealType==='buy'?'покупка':'продажа'
  // Кодируем базовые данные сделки в start-параметр: DEAL-{id}-{amount}-{currency}-{type}
  const sp = `DEAL-${d.id}-${parseFloat(d.amount)}-${d.currency}-${d.dealType}`
...
function showJoinDealModal(info) {
  _joinDealInfo = info
  const typeLabel = info.dealType === 'buy' ? '🟢 Покупка' : '🔴 Продажа'
...
function acceptJoinDeal() {
  ...
  // Если такая сделка уже есть — просто открываем
  ...
  // Создаём запись со стороны присоединившегося пользователя
  const deal = {
    id:          info.id,
    dealType:    info.dealType === 'buy' ? 'sell' : 'buy', // противоположная роль
    amount:      info.amount,
